...
const results = await Promise.all([
  fetch('/api/1').catch(e => ({ error: e, source: 'api1' })),
  fetch('/api/2').catch(e => ({ error: e, source: 'api2' })),
  fetch('/api/3').catch(e => ({ error: e, source: 'api3' }))
])
```
...
| Do | Don't |
|----|-------|
| Always handle errors | Let Promises go unhandled |
| Use try/catch with await | Ignore error cases |
| Check `response.ok` with fetch | Assume fetch always succeeds |
| Re-throw after logging | Swallow errors silently |
...
| Question | Answer |
|----------|--------|
| What is the event loop? | JavaScript's mechanism for handling async operations by checking the call stack and task queues |
| What's the difference between microtasks and macrotasks? | Microtasks (Promises) run before the next macrotask (setTimeout); all microtasks complete first |
| Why doesn't `fetch()` reject on 404? | Fetch only rejects on network failures; HTTP errors like 404 are valid responses - check `response.ok` |
| What is `Promise.all()` vs `Promise.allSettled()`? | `all()` fails fast on first rejection; `allSettled()` waits for all and returns all results |
| What is an AbortController? | An API to cancel fetch requests and other async operations using a signal |
| When should you use Web Workers? | For CPU-intensive tasks like data processing, image manipulation, or calculations that would block the main thread |
