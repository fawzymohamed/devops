...
| Question | Answer |
|----------|--------|
| What is semantic HTML? | Using HTML elements that convey meaning about content structure, like `<article>`, `<nav>`, `<header>` instead of generic `<div>` |
| Why is accessibility important? | It ensures websites are usable by people with disabilities and improves SEO, usability, and legal compliance |
| What's the difference between localStorage and sessionStorage? | localStorage persists until cleared; sessionStorage is cleared when the tab closes |
| What is ARIA? | Accessible Rich Internet Applications - attributes that enhance accessibility for dynamic content and custom widgets |
| What are Open Graph meta tags? | Meta tags that control how content appears when shared on social media platforms |
| When would you use Canvas vs SVG? | Canvas for pixel manipulation and games; SVG for scalable graphics, icons, and charts that need DOM access |
